...
const invertedLevels = Object.assign(
  {},
  ...Object.entries(logLevels).map(([a, b]) => ({[b]: a}))
);
...
function init(config) {
  ...
  const callingAppMetricName = !callingAppName
    ? "undefined"
    : callingAppName.replace(/-/g, "");
  ...
  function prepLogObject(level, logObject) {
    ...
    if (level === logLevels.debug || level <= logLevels.error) {
  ...
  return {
    ...
    error: (label, logObject) => {
      const metricLabels = {level: "error"};
      const eventName = getEventName(logObject);
      if (eventName) {
        metricLabels.eventName = eventName;
      }
      logger.error(label, prepLogObject(logLevels.error, logObject));
      logCounter.inc(metricLabels);
    },
...
function stringifier(logJson) {
  return (obj) => {
    if (logJson) {
      if (Buffer.byteLength(obj.message, "utf8") > 60 * 1024) {
        obj.message = "too big to log";
      }
      return JSON.stringify(obj);
    } else {
      let msg = obj.split("\n").join(" ");
      if (Buffer.byteLength(msg, "utf8") > 60 * 1024) {
        msg = "too big to log";
      }
      return msg;
    }
